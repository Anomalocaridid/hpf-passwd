...
# Test that $SUDO_USER takes precedence
# @data_provider user_mutability_provider
function test_dry_run_sudo_user_env_detection_::1::() {
	bashunit::mock nixos-option mock_nixos-option_integration "/dev/full" "$2"

	assert_exec "yes 'test' | SUDO_USER='hpf-sudo-user' bash src/hpf-passwd.sh --dry-run" \
		--stderr-contains "Changing hashed password file for hpf-sudo-user at /dev/full."
}
